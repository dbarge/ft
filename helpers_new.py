...
def calc_rois(images, index):
    ...
    #name = "imgGray/dc_dataset%03d.png" % index    
    #save_image_gray(name, dc)
...
#        #print masks['0']
    ...
    #print "result0: " + str(result[0].shape)
    #print "result1: " + str(result[1].shape)    
    #print "result2: " + str(result[2].shape)   
    
    #print "result0: " + str(result[0])
    #print "result1: " + str(result[1])    
...
#    print all_lines
    ...
    #print "len(all_lines): " + str(len(all_lines))
...
def get_ROIs(originals, h1s, regression_params, index):
    ...
    for i in range(num_slices):
        ...
        #o16 = originals[i].copy().astype(np.uint16)
        #h16 = h1s[i].copy().astype(np.uint16)
        ...
        #r16 = circle_smart_radius(h16, ctr)
        
        tmp = np.zeros_like(o)
        floats_draw_circle(tmp, ctr, r, 1, -1)

        #tmp16 = np.zeros_like(o16)
        #floats_draw_circle(tmp16, ctr, r16, 1, -1)
        
        result = tmp*o
        
        #results16.append(tmp16 * o16)        
        ...
        name = "imgGray/tmp_case%03d_slice%02d_color.png" % (index, i)
        save_image_gray(name, tmp.astype(np.uint16))

 
        #-----------------------------------------------------------------------------------------------------------------------------------------------------
        #-----------------------------------------------------------------------------------------------------------------------------------------------------

        name = "imgGray/orig_case%03d_slice%02d_color.png" % (index, i)
        save_image_gray(name, o.astype(np.uint16))
    
    
        #-----------------------------------------------------------------------------------------------------------------------------------------------------
        #-----------------------------------------------------------------------------------------------------------------------------------------------------

        name = "imgGray/roi_case%03d_slice%02d_color.png" % (index, i)
        save_image_gray(name, result.astype(np.uint16))    
    
    
    #results2 = cv2.convertScaleAbs(np.array(results16))
    #results2 = np.array( cv2.convertScaleAbs(results16) )
    
    #return (results2, np.array(circles))    
...
def find_best_angle(roi, circ):
    ...
    def score_matched(trimx, trimy):
        ...
        filt = 1 - ((np.cos(COS_MATCHED_FILTER_FREQ*2*np.pi*newtrimx)) / 2 + (0.5))
...
def find_threshold_point(best, best_fn):
    ...
    restrict = dom[np.max(closest_min-THRESHOLD_AREA, 0): closest_min+THRESHOLD_AREA]
...
def locate_lv_component(img_bin, coords, idx):
    ...
    #print "Connected Components: " + str(num)
